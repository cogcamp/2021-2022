...
mainScene.create = function() {
    ...
    // ライフのテキスト表示
    this.lifeText = this.add.text(30,20, 'My Life is' + " " + this.life,{
...
mainScene.config = function() {
    // 背景色の設定
    this.cameras.main.setBackgroundColor('#00ff7f');
...
mainScene.createBlocks = function() {
    // 横10列、縦6行並べる
    //ブロックの色の配列
    var blockColors = [ 'red1','green1' , 'yellow1','silver1','blue1','purple1','silver1' ];
...
mainScene.gameClear = function() {
    // ゲームクリア
    alert("amazing! The game is clear(o^―^o)smile");
    // スタートシーンに移動
...
mainScene.failToHit =  function () {
    ...
    // ライフを減らす
    this.life--;
    this.lifeText.text = 'My Life is' + " "  + this.life;
    // ライフが0になると
    if(this.life <= 0) {
        // 0.5秒後にゲームオーバー
        this.time.addEvent({
...
mainScene.gameOver = function() {
    // ゲームオーバー
    alert("That's too bad!(^^)!The game is over！");
    // スタートシーンに移動
    this.scene.start("startScene");
};
